...
export function Home() {
  ...
  const ocupacaoGlobal = useMemo(() => {
    ...
    proximosPasseios.forEach((p) => {
      ...
      const alocados = passageiros.filter(pax => pax.passeioId === p.id && !pax.isCriancaColo).length
        ...
        {proximosPasseios.length === 0 ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-5">
            {proximosPasseios.map((passeio) => {
              ...
              const alocados = passageiros.filter((p) => p.passeioId === passeio.id && !p.isCriancaColo).length
              const faltam = Math.max(0, capacidade - alocados)
